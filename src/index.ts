...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'data.cityofchicago.org Socrata');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'datasets': {
      ...
      const res = await pwFetch(`http://api.us.socrata.com/api/catalog/v1?${p}`, { headers: { Accept: 'application/json', 'User-Agent': UA } });
...
async function socrataGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw await httpError(res, 'data.cityofchicago.org');
  return res.json();
}
